...
import 'package:koa_hang_app/data/BenefitTitle.dart' as _benefiTitle;
import 'package:koa_hang_app/data/BenefitDetail.dart' as _benefitDetail;
...
Future<List<_benefiTitle.BenefitTitle>> fetchTitle() async {
  try {
    http.Response response =
    await http.get(Server().IPAddress + '/benefit/getTitle.php');
    List responseJson = json.decode(response.body);
    //print(response.body);
    return responseJson.map((m) => new _benefiTitle.BenefitTitle.fromJson(m)).toList();
  }catch (exception) {
    print(exception.toString());
  }
}

Future<List<_benefitDetail.BenefitDetail>> fetchDetail(id) async {
  try {
    http.Response response =
    await http.get(Server().IPAddress + '/benefit/getDetail.php?id='+id);
    List responseJson = json.decode(response.body);
    //print(response.body);
    return responseJson.map((m) => new _benefitDetail.BenefitDetail.fromJson(m)).toList();
  }catch (exception) {
    print(exception.toString());
  }
}
...
final makeBodyTitle = FutureBuilder<List<_benefiTitle.BenefitTitle>>(
    future: fetchTitle(),
    builder: (context, snapshot) {
      ...
      List<_benefiTitle.BenefitTitle> benefit = snapshot.data;
                ...
                child: ListTile(
                  ...
                  subtitle: Row(
                    children: <Widget>[
                      ...
                      Text("ล่าสุดเมื่อวันที่ : "+benefit[index].benefit_date,
                  ...
                  Icon(Icons.keyboard_arrow_right, color: cl_img_icon,
                      size: 28.0),
                  onTap: () => _onTapItem(context, benefit[index].benefit_id),
                ),
              ),
            );
          },
...
void _onTapItem(BuildContext context,id) {
  final makeBodyDetail = FutureBuilder<List<_benefitDetail.BenefitDetail>>(
      ...
      builder: (context, snapshot) {
        ...
        // Shows the real data with the data retrieved.
        List<_benefitDetail.BenefitDetail> benefit = snapshot.data;
        List splits = benefit[0].benefit_img.split(",");
                    ...
                    child: new ConstrainedBox(
                      ...
                      child: new Column(children: <Widget>[
                        ...
                        new Container(
                          ...
                          //color: Colors.grey,
                          child: new Text(
                            'ชื่อผลิตภัณฑ์ : ' + benefit[0].benefit_name,
                            style: TextStyle(color: cl_text_title,
                                fontSize: 20.0,
                                fontFamily: 'Poppins'),
                          ),
                        ...
                        new Container(
                          ...
                          child: new Text(
                            'รายละเอียด : ' + benefit[0].benefit_detail,
                            ...
                            Padding(
                              ...
                              child: RaisedButton(
                                child: MaterialButton(
                                  ...
                                  onPressed: () {
                                    vdoPlay(benefit[0].benefit_vdo, context);
                                  },
                                ),
                              ),
                            )
  ...
  final makeAppBar = FutureBuilder<List<_benefitDetail.BenefitDetail>>(
      future: fetchDetail(id),
      builder: (context, snapshot) {
        if (!snapshot.hasData)
          // Shows progress indicator until the data is load.
        ...
        // Shows the real data with the data retrieved.
        List<_benefitDetail.BenefitDetail> benefit = snapshot.data;
        return new Text(benefit[0].benefit_detail,style: TextStyle(fontFamily: 'Poppins'),);
