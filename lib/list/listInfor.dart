...
import 'package:koa_hang_app/data/InformationTitle.dart' as _inforTitle;
import 'package:koa_hang_app/data/InformationDetail.dart' as _inforDetail;
...
Future<List<_inforTitle.InformationTitle>> fetchTitle() async {
  try {
    http.Response response =
    await http.get(Server().IPAddress + '/infor/getTitle.php');
    List responseJson = json.decode(response.body);
    //print(response.body);
    return responseJson.map((m) => new _inforTitle.InformationTitle.fromJson(m)).toList();
  }catch (exception) {
    print(exception.toString());
  }
}

Future<List<_inforDetail.InformayionDetail>> fetchDetail(id) async {
  try {
    http.Response response =
    await http.get(Server().IPAddress + '/infor/getDetail.php?id='+id);
    List responseJson = json.decode(response.body);
    //print(response.body);
    return responseJson.map((m) => new _inforDetail.InformayionDetail.fromJson(m)).toList();
  }catch (exception) {
    print(exception.toString());
  }
}
...
final makeBodyTitle = FutureBuilder<List<_inforTitle.InformationTitle>>(
    future: fetchTitle(),
    builder: (context, snapshot) {
      ...
      List<_inforTitle.InformationTitle> infor = snapshot.data;
                ...
                child: ListTile(
                  ...
                  subtitle: Row(
                    children: <Widget>[
                      ...
                      Text("ล่าสุดเมื่อวันที่ : "+infor[index].infor_date,
                  ...
                  Icon(Icons.keyboard_arrow_right, color: cl_img_icon,
                      size: 28.0),
                  onTap: () => _onTapItem(context,infor[index].infor_content, infor[index].infor_id),
                ),
              ),
            );
          },
...
void _onTapItem(BuildContext context,title,id) {
  final makeImg = FutureBuilder<List<_inforDetail.InformayionDetail>>(
      ...
      builder: (context, snapshot) {
        ...
        // Shows the real data with the data retrieved.
        List<_inforDetail.InformayionDetail> infor = snapshot.data;
        List splits = infor[0].infor_img.split(",");
        for (int i = 0; i < splits.length; i++) {
          return new Image.network(splits[i],
              fit: BoxFit.cover
          );
        }
      }
  ...
  final makeBodyDetail = FutureBuilder<List<_inforDetail.InformayionDetail>>(
      future: fetchDetail(id),
      builder: (context, snapshot) {
        if (!snapshot.hasData)
          // Shows progress indicator until the data is load.
                    ...
                    child: new ConstrainedBox(
                      ...
                      child: new Column(children: <Widget>[
                        ...
                        new Container(
                          ...
                          //color: Colors.grey,
                          child: new Text(
                            'หัวเรื่อง : ' + infor[0].infor_content,
                            style: TextStyle(color: cl_text_title,
                                fontSize: 20.0,
                                fontFamily: 'Poppins'),
                          ),
                            ...
                            Padding(
                              ...
                              child: RaisedButton(
                                child: MaterialButton(
                                  ...
                                  onPressed: () {
                                    vdoPlay(infor[0].infor_vdo, context);
                                  },
                                ),
                              ),
                            )
  ...
  final makeAppBar = FutureBuilder<List<_inforDetail.InformayionDetail>>(
      future: fetchDetail(id),
      builder: (context, snapshot) {
        if (!snapshot.hasData)
          // Shows progress indicator until the data is load.
        ...
        // Shows the real data with the data retrieved.
        List<_inforDetail.InformayionDetail> infor = snapshot.data;
        return new Text(infor[0].infor_content,style: TextStyle(fontFamily: 'Poppins'),);
